...
def _safe_path_component(value: str) -> str:
    """Make a CLI/eval/model label safe to use in a filename."""
    safe = re.sub(r"[^A-Za-z0-9._-]+", "-", str(value)).strip(".-")
    return safe or "unknown"


def default_run_paths(
    eval_name: str,
    model_name: str,
    run_id: str,
    *,
    root: Union[str, Path] = "experiments/runs",
) -> Tuple[str, str]:
    """Return paired default JSONL and log paths for one evaluation run."""
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    stem = "__".join(
        _safe_path_component(value) for value in (eval_name, model_name, run_id)
    )
    return str(root_path / f"{stem}.jsonl"), str(root_path / f"{stem}.log")


def _attach_file_log_handler(path: str) -> None:
    """Attach a file handler after the run id is available."""
    log_path = Path(path)
...
def run(args: OaiEvalArguments, registry: Optional[Registry] = None) -> str:
    ...
    model_names = [getattr(fn, "model", None) for fn in completion_fn_instances]
    model_name = "+".join(name for name in model_names if name) or args.completion_fn
    default_record_path, default_log_path = default_run_paths(
        args.eval, model_name, run_spec.run_id
    )
    record_path = args.record_path or default_record_path
    if args.log_to_file is None:
        args.log_to_file = default_log_path
        _attach_file_log_handler(args.log_to_file)

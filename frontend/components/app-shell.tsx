...
export function AppShell({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <div className="app-frame"><a className="skip-link" href="#main-content">{t("skipToContent")}</a><aside className="sidebar"><Link className="brand" href="/app"><span className="brand-mark">M</span>Medical Evals</Link><nav aria-label="Main navigation"><Link className={pathname === "/app" ? "nav-link active" : "nav-link"} href="/app"><GridIcon size={17} />{t("overview")}</Link><Link className={pathname.startsWith("/app/models") ? "nav-link active" : "nav-link"} href="/app/models"><ListIcon size={17} />{t("models")}</Link><Link className={pathname.startsWith("/app/evaluations") ? "nav-link active" : "nav-link"} href="/app/evaluations"><ListIcon size={17} />{t("evaluations")}</Link></nav><div className="sidebar-footer"><span className="sidebar-caption">{user?.username} · {user?.role}</span><button className="locale-toggle" onClick={() => setLocale(locale === "en" ? "zh" : "en")}><GlobeIcon size={17} />{t("language")}</button><button className="logout" onClick={() => { void logout(); router.replace("/admin/login"); }}><LogoutIcon size={17} />{t("signOut")}</button></div></aside><section className="app-content" id="main-content">{children}</section></div>;
}

...
type SampleRecord = { index: number; sample_id: string; predicted?: string | null; raw_output?: string; correct?: boolean; parse_failed?: boolean; error?: string | null; score?: number; rubric_judgments?: RubricJudgment[] };
...
export default function ResultsPage() {
  ...
  const load = useCallback(async (includeSamples = true) => {
    ...
    setSummary(result); setRunLog(log); if (includeSamples) await loadSamples(0);
  }, [loadSamples, params.id]);
  useEffect(() => { load().catch(() => setError(t("unableLoadResults"))); }, [load, t]);
  useEffect(() => { if (!isLive) return; const timer = window.setInterval(() => { load(true).catch(() => undefined); }, 3000); return () => window.clearInterval(timer); }, [isLive, load]);

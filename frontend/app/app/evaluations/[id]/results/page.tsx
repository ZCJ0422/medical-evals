...
import { api, apiBlob, apiText, ApiError } from "../../../../../lib/api";
...
export default function ResultsPage() {
  ...
  const [summary, setSummary] = useState<ResultSummary | null>(null); const [samples, setSamples] = useState<SampleRecord[]>([]); const [sampleTotal, setSampleTotal] = useState(0); const [runLog, setRunLog] = useState(""); const [error, setError] = useState(""); const [reportState, setReportState] = useState<"idle" | "busy">("idle"); const [reportError, setReportError] = useState(""); const [copied, setCopied] = useState<string | null>(null); const [filter, setFilter] = useState<"all" | "issues" | "low">("all"); const [followLogs, setFollowLogs] = useState(true); const logRef = useRef<HTMLPreElement>(null); const sampleTotalRef = useRef(0); const samplesLoadedRef = useRef(false);
  ...
  const load = useCallback(async (includeSamples = true) => {
    const [result, log] = await Promise.all([
      api<any>(`/api/v1/evaluations/${params.id}/summary`),
      apiText(`/api/v1/evaluations/${params.id}/artifacts/run.log`).catch((cause) => {
        if (cause instanceof ApiError && cause.status === 404) return "";
        throw cause;
      }),
    ]);
    const normalized = { ...result, task_id: result.run_id, stage: result.progress?.stage, progress_percent: result.progress?.progress_percent };
    setSummary(normalized); setRunLog(log); if (includeSamples) await loadSamples(0); return normalized;
  }, [loadSamples, params.id]);
  const retryLoad = useCallback(() => { setError(""); void load().catch(() => setError(t("unableLoadResults"))); }, [load, t]);
  ...
  async function generateReport() {
    setReportState("busy"); setReportError("");
    try {
      await api(`/api/evaluations/${params.id}/report/generate`, { method: "POST" });
      const blob = await apiBlob(`/api/v1/evaluations/${params.id}/artifacts/report.html`);
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a"); link.href = url; link.download = "report.html"; link.click();
      window.setTimeout(() => URL.revokeObjectURL(url), 1000);
    } catch { setReportError(t("reportFailed")); }
    finally { setReportState("idle"); }
  }

  if (error) return <main className="shell"><InlineAlert tone="error"><span>{error}</span> <button type="button" className="inline-button" onClick={retryLoad}>{t("retryLoad")}</button></InlineAlert><Link href="/app/evaluations" className="back-link"><ArrowLeftIcon size={16} />{t("backToEvaluations")}</Link></main>;

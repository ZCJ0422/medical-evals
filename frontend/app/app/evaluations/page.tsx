...
export default function EvaluationsPage() {
  ...
  async function performDelete(taskId: string) { try { await api(`/api/evaluations/${taskId}`, { method: "DELETE" }); setTasks((items) => items.filter((item) => item.task_id !== taskId)); setNotice(t("deleted")); } catch (cause) { setError(cause instanceof Error ? cause.message : t("unableLoadTasks")); } }
  ...
  async function cancelTask(taskId: string) { if (!window.confirm(t("cancelConfirm"))) return; try { const updated = await api<TaskSummary>(`/api/evaluations/${taskId}/cancel`, { method: "POST" }); setTasks((items) => items.map((item) => item.task_id === taskId ? updated : item)); setNotice(t("cancelled")); } catch (cause) { setError(cause instanceof Error ? cause.message : t("unableLoadTasks")); } }
  async function retryTask(taskId: string) { if (!window.confirm(t("retryConfirm"))) return; try { const created = await api<TaskSummary>(`/api/evaluations/${taskId}/retry`, { method: "POST" }); setTasks((items) => [created, ...items]); setNotice(t("retryCreated")); } catch (cause) { setError(cause instanceof Error ? cause.message : t("unableLoadTasks")); } }
  async function resumeTask(taskId: string) { if (!window.confirm(t("resumeConfirm"))) return; try { const updated = await api<TaskSummary>(`/api/evaluations/${taskId}/resume`, { method: "POST" }); setTasks((items) => items.map((item) => item.task_id === taskId ? updated : item)); setNotice(t("resumed")); } catch (cause) { setError(cause instanceof Error ? cause.message : t("unableLoadTasks")); } }
  const load = useCallback(async (manual = false) => { if (manual) setRefreshing(true); try { setTasks(await api<TaskSummary[]>("/api/evaluations")); setError(""); } catch { setError(t("unableLoadTasks")); } finally { setLoading(false); setRefreshing(false); } }, [t]);

...
export default function PublicPortalPage() {
  ...
  async function submitEvaluation(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const created = await api<EvaluationRun>("/api/v1/evaluations", { method: "POST", body: JSON.stringify({ name: "", evaluation_definition_id: selectedDefinition.id, target_model_id: model.id, judge_model_id: "", split: selectedSplit.id, sample_limit: selectedSplit.default_sample_limit, config: {} }) });
  ...
  return <main className="public-site portal-page">
    ...
    <div className="portal-workspace">
      <aside className="portal-sidebar" aria-label="测评空间菜单">
        ...
        <button className={view === "records" ? "portal-sidebar-item active" : "portal-sidebar-item"} onClick={() => setView("records")}>提交记录</button>
      </aside>

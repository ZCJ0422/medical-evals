...
async function prepareAuthenticatedPage(page: import("@playwright/test").Page) {
  await page.addInitScript(() => {
    window.sessionStorage.setItem("medical_evals_token", "e2e-token");
...
test("HealthBench sample records keep the answer in the scrollable panel only", async ({ page }) => {
  ...
  await page.route("**/api/evaluations/healthbench-e2e/results", (route) => route.fulfill({ json: summary(1) }));
  await page.route("**/api/evaluations/healthbench-e2e/log", (route) => route.fulfill({ body: "run started\n", contentType: "text/plain" }));
  await page.route("**/api/evaluations/healthbench-e2e/samples?limit=50&offset=0", (route) => route.fulfill({ json: { task_id: "healthbench-e2e", offset: 0, limit: 50, total: 1, samples: [sample(0)] } }));

  await page.goto("/app/evaluations/healthbench-e2e/results");
  const record = page.locator(".sample-record");
  await expect(record).toBeVisible();
  await expect(record.locator(".sample-raw-heading")).not.toContainText("Seek care if symptoms worsen.");
...
test("running HealthBench results refresh samples only when the persisted count changes", async ({ page }) => {
  ...
  let sampleCalls = 0;
  await page.route("**/api/evaluations/healthbench-live/results", (route) => {
    return route.fulfill({ json: { ...summary(persistedCount), status: "running", progress_percent: persistedCount * 50, stage: "target_model" } });
  });
  await page.route("**/api/evaluations/healthbench-live/log", (route) => route.fulfill({ body: "running\n", contentType: "text/plain" }));
  await page.route("**/api/evaluations/healthbench-live/samples?limit=50&offset=0", (route) => {
    sampleCalls += 1;

...
export type EvaluationDefinitionSplit = { id: string; dataset_version_id: string; version: string; sample_count: number; default_sample_limit: number; source_path?: string | null; source_sha256?: string | null };
...
export type EvaluationRun = { run_id: string; submitted_by?: string | null; task_id?: string; name: string; evaluation_definition_id: string; target_model_id: string; judge_model_id: string; dataset_version_id: string; status: EvaluationStatus; progress: TaskProgress; split: string; sample_limit: number | null; config: Record<string, unknown>; created_at: string; updated_at: string; queued_at: string; started_at: string | null; finished_at: string | null; retry_of_run_id: string | null; error: string | null };
export type TaskSummary = EvaluationRun;
export type MonitoringMetrics = { generated_at: string; queue_depth: number | null; queue_pending: number | null; queue_error: string | null; status_counts: Record<string, number>; expired_leases: number; failure_categories: Record<string, number>; request_success_count: number; parse_failed_count: number; parse_success_rate: number | null; artifact_bytes: number; artifact_file_count: number };

...
def dataset_source_path(dataset_version_id: str):
    """Return the repository-relative source path for a registered dataset version."""
    sources = {
        "medical-medqa.dev.v1": ("registry/data/medical_medqa/dev.jsonl", registry_data_path("medical_medqa", "dev.jsonl")),
        "medical-healthbench.smoke.v1": ("registry/data/medical_healthbench/smoke.jsonl", registry_data_path("medical_healthbench", "smoke.jsonl")),
        "medical-healthbench.oss.v1": ("dataset/HealthBench/2025-05-07-06-14-12_oss_eval.jsonl", workspace_dataset_path("HealthBench", "2025-05-07-06-14-12_oss_eval.jsonl")),
        "medical-healthbench.hard.v1": ("dataset/HealthBench/hard_2025-05-08-21-00-10.jsonl", workspace_dataset_path("HealthBench", "hard_2025-05-08-21-00-10.jsonl")),
        "medical-healthbench.consensus.v1": ("dataset/HealthBench/consensus_2025-05-09-20-00-46.jsonl", workspace_dataset_path("HealthBench", "consensus_2025-05-09-20-00-46.jsonl")),
    }
    try:
        relative, path = sources[dataset_version_id]
    except KeyError as error:
        raise ValueError(f"Unsupported dataset version: {dataset_version_id}") from error
...
def healthbench_samples_path(dataset_version_id: str):
    ...
    relative, path = dataset_source_path(dataset_version_id)
    if not dataset_version_id.startswith("medical-healthbench"):
        raise ValueError(f"Unsupported HealthBench dataset version: {dataset_version_id}")
    return path

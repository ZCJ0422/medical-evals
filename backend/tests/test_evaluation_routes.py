...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        config.settings,
        "database_url",
        f"sqlite+pysqlite:///{tmp_path / 'tasks.sqlite3'}",
    )
...
def test_create_evaluation_requires_admin(client):
...
def test_create_evaluation_returns_queued_task(client):
    response = client.post(
        "/api/evaluations",
...
def test_create_medqa_without_judge_and_name_uses_generated_name(client):
    response = client.post(
        "/api/evaluations",
        headers=admin_headers(client),
        json={
...
def test_healthbench_requires_judge_configuration(client):
    response = client.post(
...
def test_create_direct_api_key_is_encrypted_and_not_returned(client):
    response = client.post(
        "/api/evaluations",
        headers=admin_headers(client),
        json={
    ...
    row = client.get(f"/api/evaluations/{task_id}", headers=admin_headers(client))
    assert "target-secret" not in row.text


def test_create_evaluation_accepts_explicit_credential_environment_names(client):
    from medical_evals_api import config
    ...
    task_id = response.json()["task_id"]
    repo = __import__(
        "medical_evals_api.repositories.tasks", fromlist=["TaskRepository"]
    ).TaskRepository(config.settings.database_path)
    task = repo.get(task_id)
    assert task is not None
    assert task.target_api_key_env == "TARGET_KEY"


def test_preflight_rejects_invalid_credential_environment_name(client):
    response = client.post(
        "/api/evaluations/preflight",
        headers=admin_headers(client),
        json={
...
def test_retry_completed_evaluation_creates_new_queued_task(client):
    from medical_evals_api import config
    ...
    repo.set_status(original["task_id"], TaskStatus.COMPLETED)

    retried = client.post(
        f"/api/evaluations/{original['task_id']}/retry",
        headers=admin_headers(client),
    )
    ...
    retried_again = client.post(
        f"/api/evaluations/{retried.json()['task_id']}/retry",
        headers=admin_headers(client),
    )
...
def test_resume_partial_evaluation_reuses_same_task(client):
    from medical_evals_api import config
    ...
    resumed = client.post(
        f"/api/evaluations/{task_id}/resume",
        headers=admin_headers(client),
    )
...
def test_cancel_running_evaluation_updates_status(client):
    from medical_evals_api import config
    ...
    repo.set_status(task_id, TaskStatus.RUNNING)

    cancelled = client.post(
        f"/api/evaluations/{task_id}/cancel",
        headers=admin_headers(client),
    )
...
def test_preflight_rejects_api_key_entered_as_base_url(client):
    response = client.post(
        "/api/evaluations/preflight",
        headers=admin_headers(client),
        json={
...
def test_preflight_rejects_unknown_dataset_and_mismatched_rubric(client):
    response = client.post(
        "/api/evaluations/preflight",
        headers=admin_headers(client),
        json={
    ...
    assert any(
        "Rubric must be healthbench-default" in error
        for error in mismatched.json()["errors"]
    )
